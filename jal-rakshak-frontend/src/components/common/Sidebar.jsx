...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Horizontal Command Bar (< lg) */}
      <div className="lg:hidden bg-slate-900 border-b border-slate-800 px-3 py-2 sticky top-16 z-30 shadow-md">
        <div className="flex items-center justify-between gap-2 pb-1.5 border-b border-slate-800/60">
          <div className="flex items-center gap-2">
            ...
            >
              {isRescue ? <LifeBuoy className="w-3.5 h-3.5" /> : <Shield className="w-3.5 h-3.5" />}
            </span>
            <span className="text-xs font-bold text-white truncate">
              {isRescue ? 'NDRF Rescue Unit' : 'SEOC State Control'}
            </span>
          </div>
            ...
            return (
              ...
              >
                ...
                {link.badge !== undefined && link.badge > 0 && (
                  <span className={`px-1.5 py-0.2 rounded-full text-[9px] font-black ${link.badgeColor}`}>
                    {link.badge}
                  </span>
                )}
              </Link>
      ...
      <aside className="hidden lg:flex w-64 bg-slate-900 text-slate-100 flex-col border-r border-slate-800 shrink-0 min-h-[calc(100vh-4rem)]">
        {/* Portal Header Badge */}
